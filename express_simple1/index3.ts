...
controllers.forEach((controller) => {
    ...
    Object.getOwnPropertyNames(controllerPrototype).forEach((key) => {
        ...
        const path = Reflect.getMetadata('path', controllerPrototype, key);

        if (method && path) {
            console.log(`Registering route: [${method.toUpperCase()}] ${basePath}${path}`);
            (router as any)[method](`${basePath}${path}`, (req: Request, res: Response) => {
                console.log(`Handling request: [${method.toUpperCase()}] ${basePath}${path}`);
